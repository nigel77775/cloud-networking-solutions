...
Usage:
    # Create a new agent
    python deploy_agent.py --project=PROJECT_ID \
        --dms-mcp-url=https://dms.internal.ai-demo.gcp.sc-ccn.xyz/mcp \
        --income-verification-url=https://income-verification.internal.ai-demo.gcp.sc-ccn.xyz \
        --email-mcp-url=https://email.internal.ai-demo.gcp.sc-ccn.xyz/mcp
...
def main() -> None:
    ...
    parser.add_argument(
        "--dms-mcp-url",
        default=None,
        help="DMS MCP server URL (e.g. https://dms.internal.ai-demo.gcp.sc-ccn.xyz/mcp). "
        "Required unless --ge-deploy-only is used.",
    )
    parser.add_argument(
        "--income-verification-url",
        default=None,
        help="Income Verification API base URL; /mcp is appended automatically "
        "(e.g. https://income-verification.internal.ai-demo.gcp.sc-ccn.xyz). "
        "Required unless --ge-deploy-only is used.",
    )
    parser.add_argument(
        "--email-mcp-url",
        default=None,
        help="Corporate Email MCP server URL (e.g. https://email.internal.ai-demo.gcp.sc-ccn.xyz/mcp). "
        "Required unless --ge-deploy-only is used.",
    )
    parser.add_argument(
        "--region",
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Disable SSL certificate verification for the agent (insecure)",
    )
    parser.add_argument(
        "--model",
    ...
    parser.add_argument(
        "--agent-location",
        default="us-central1",
        help="Location for the agent's environment (default: us-central1)",
    )
    parser.add_argument(
        "--agent-name",
    ...
    # Full deploy requires MCP URLs
    if not args.dms_mcp_url:
        parser.error("--dms-mcp-url is required (unless using --ge-deploy-only)")
    if not args.income_verification_url:
        parser.error("--income-verification-url is required (unless using --ge-deploy-only)")
    if not args.email_mcp_url:
        parser.error("--email-mcp-url is required (unless using --ge-deploy-only)")
    ...
    print(f"  DMS MCP URL:    {args.dms_mcp_url}")
    print(f"  Income Ver URL: {args.income_verification_url}")
    print(f"  Email MCP URL:  {args.email_mcp_url}")
    print(f"  Model:          {args.model}")
    print(f"  Display name:   {args.display_name}")
    print(f"  Staging bucket: {staging_bucket}")
    print(f"  Mode:           {'update' if args.update else 'create'}")
    print(f"  Agent Location: {args.agent_location}")
    ...
    # Set service URLs so the agent module can read them at import time.
    income_verification_mcp_url = args.income_verification_url.rstrip("/") + "/mcp"
    os.environ["DMS_MCP_URL"] = args.dms_mcp_url
    os.environ["INCOME_VERIFICATION_URL"] = income_verification_mcp_url
    os.environ["EMAIL_MCP_URL"] = args.email_mcp_url
    ...
    try:
        ...
        deploy_config = dict(
            ...
            env_vars={
                ...
                "DISABLE_SSL_VERIFICATION": "true" if args.disable_ssl_verification else "false",
                "GOOGLE_API_PREVENT_AGENT_TOKEN_SHARING_FOR_GCP_SERVICES": "false",
                "OTEL_INSTRUMENTATION_GENAI_CAPTURE_MESSAGE_CONTENT": "true",
                "OTEL_TRACES_SAMPLER": "parentbased_traceidratio",
                "OTEL_TRACES_SAMPLER_ARG": "1.0",
                "DMS_MCP_URL": args.dms_mcp_url,
                "INCOME_VERIFICATION_URL": income_verification_mcp_url,
                "EMAIL_MCP_URL": args.email_mcp_url,
                "GOOGLE_GENAI_USE_VERTEXAI": "True",
                "GOOGLE_CLOUD_LOCATION": args.agent_location,
