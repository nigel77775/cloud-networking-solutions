...
output "global_certificate_id" {
  ...
  value       = var.enable_certificate_manager && var.dns_zone_domain != null ? google_certificate_manager_certificate.global[0].id : null
}
...
output "global_dns_authorizations" {
  ...
  value = var.enable_certificate_manager && var.dns_zone_domain != null ? {
    "${replace(trimsuffix(var.dns_zone_domain, "."), ".", "-")}-global" = google_certificate_manager_dns_authorization.global[0]
  } : {}
}
