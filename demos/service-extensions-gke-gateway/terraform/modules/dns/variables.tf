...
variable "certificate_dns_authorizations_global" {
  description = "DNS authorizations from global certificates (output from module.certificates.global_dns_authorizations)"
  type = map(object({
    id     = string
    domain = string
    dns_resource_record = list(object({
