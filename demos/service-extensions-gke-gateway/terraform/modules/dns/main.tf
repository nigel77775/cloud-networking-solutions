...
# Certificate validation DNS records for global certificates
resource "google_dns_record_set" "certificate_validation_global" {
  for_each = var.enable_certificate_manager && var.dns_zone_domain != null && var.certificate_dns_authorizations_global != null ? var.certificate_dns_authorizations_global : {}
...
# DNS wildcard A record for *.internal.example.com
